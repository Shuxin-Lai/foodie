...
public class DefaultWorkerIdStrategy implements WorkerIdStrategy {
  ...
  private final Properties props = Props.tryProperties(
    "idworker-client.properties",
    Utils.DOT_IDWORKERS
  );
  private final String idWorkerServerUrl = props.getProperty(
    "server.address",
  ...
  private long increaseWithWorkerIdServer() {
    String incId = HttpReq
      .get(idWorkerServerUrl)
      .req("/inc")
  ...
  private long checkAvail(long wid) {
    ...
    try {
      ...
      new File(idWorkerHome, ipudotlock + String.format("%04d", wid))
        .createNewFile();
