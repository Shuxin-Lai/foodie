...
public class DefaultRandomCodeStrategy implements RandomCodeStrategy {
  ...
  public void init() {
    ...
    throw new RuntimeException(
      "all prefixes are used up, the world maybe ends!"
    );
  }
  ...
  protected boolean tryUsePrefix() {
    codePrefixIndex =
      new File(idWorkerHome, Id.getWorkerId() + ".code.prefix." + prefixIndex);
  ...
  private boolean createBloomFilter() {
    ...
    if (codesFilter == null) {
      ...
      if (size >= MAX_BITS) {
        log.warn(
          "bloom filter with prefix file {} is already full",
          codePrefixIndex
        );
        return false;
      }
  ...
  private void generate() {
    for (int i = 0; i < CACHE_CODES_NUM; ++i) availableCodes.add(generateOne());
